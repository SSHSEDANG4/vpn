...
clear
non-ssl="$(cat ~/log-install.txt | grep -w "Websocket NON SSL" | cut -d: -f2|sed 's/ //g')"
echo -e "======================================" | lolcat
...
if [[ -z $cek ]]; then
sed -i "s/$non-ssl/$stl/g" nano /usr/local/bin/ws-dropbear
sed -i "s/   - Websocket NON SSL       : $non-ssl/   - Websocket NON SSL       : $stl/g" /root/log-install.txt
