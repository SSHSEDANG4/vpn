...
portsshttp="$(cat ~/log-install.txt | grep -w "SS-OBFS TLS" | cut -d: -f2|sed 's/ //g')"
portssr="$(cat ~/log-install.txt | grep -w "SS-OBFS TLS" | cut -d: -f2|sed 's/ //g')"
